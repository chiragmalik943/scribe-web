...
(function () {
  ...
  var FRAGMENTS = [
    { said: '400 seats',            short: '400 seats',      kind: 'risk',     from: 'Commercials · 2 Sep',
      read: 'The July pricing sheet still says 250. Two numbers, one contract.' },
    { said: 'March go-live',        short: 'March go-live',  kind: 'decision', from: 'Kick-off · 14 Jul',
      read: 'Agreed on the kick-off and never revisited. Migration depends on it.' },
    { said: 'Who owns SSO?',        short: 'Who owns SSO?',  kind: 'question', from: 'Technical · 4 Aug',
      read: 'Asked three calls ago. Nobody has answered it since.' },
    { said: 'Send pricing by Friday', short: 'Pricing Friday', kind: 'task',   from: 'Commercials · 2 Sep',
      read: 'Owed by Karen, due Friday. Nothing has been sent yet.' },
    { said: 'Legal must clear DPA', short: 'Legal clears DPA', kind: 'risk',   from: 'Security · 21 Aug',
      read: 'Sits before the March date and has no owner on it.' },
    { said: 'Uplift holds at 4%',   short: 'Uplift 4%',      kind: 'decision', from: 'Commercials · 2 Sep',
      read: 'The renewals note from June says 6%. Both are on record.' },
    { said: '60 clinicians',        short: '60 clinicians',  kind: 'fact',     from: 'Discovery · 9 Jul',
      read: 'Quoted as the pilot group every time it has come up.' },
    { said: 'Deep-dive on the 25th', short: 'Deep-dive 25th', kind: 'task',    from: 'Technical · 4 Aug',
      read: 'Scheduled, but no agenda and no invite has gone out.' },
    { said: 'Backfills too?',       short: 'Backfills too?', kind: 'question', from: 'Discovery · 9 Jul',
      read: 'Raised once, answered never — and it changes the seat count.' },
    { said: 'Security review first', short: 'Security first', kind: 'risk',    from: 'Security · 21 Aug',
      read: 'Blocks the pilot, and the pilot is what March depends on.' },
    { said: 'Two-year term',        short: 'Two-year term',  kind: 'decision', from: 'Commercials · 2 Sep',
      read: 'Settled. Every later call has been consistent with it.' },
    { said: 'Revisit at 90 days',   short: 'Revisit at 90d', kind: 'decision', from: 'Kick-off · 14 Jul',
  ...
  /* ── state ───────────────────────────────────────────────────────────── */
  var W = 0, H = 0, dpr = 1;
  var small = false;     // the phone-shaped lane composition
  var compact = false;   // short labels — the ring has no room for long ones
  ...
  var flow = 0, auto = -1;   // the narrow layout: 0–1 position in the drift, and what it is absorbing
  ...
  function layout() {
    ...
    /* Two compositions, because a ring of text pills cannot be read at phone
       width. Wide: the project card in the middle of an orbiting ring. Narrow:
       the card at the foot of a lane, with fragments drifting down into it —
       the same claim, told as absorption instead of orbit. */
    /* Which composition applies is read off the frame's own shape rather than a
       width, so the CSS breakpoint and this decision can never disagree: a
       frame taller than three-quarters of its width is a phone-shaped one. */
    small = H / W > 0.75;

    /* Below about 900px the ring's radius no longer clears a long pill, so the
       fragments switch to their short form. The card always quotes the line in
       full, so nothing is lost — only the pill gets shorter. */
    compact = small || W < 900;

    if (small) {
      card.w = Math.min(W - 20, 340);
      card.h = 172;
      card.x = W / 2;
      card.y = H - card.h / 2 - 10;
    } else {
      card.w = clamp(W * 0.34, 236, 340);
      card.h = 190;
      card.x = W / 2;
      card.y = H / 2;
      ringX = Math.min((W - card.w) / 2 + card.w * 0.20, W / 2 - 24);
      ringY = H * 0.33;
    }

    var count = small ? 6 : FRAGMENTS.length;
    if (items.length !== count) {
      ...
      for (var i = 0; i < count; i++) {
        ...
        items.push({
          ...
          phase: (i / count) * TAU,
          lift: (((i * 5) % count) / (count - 1) - 0.5) * 0.46,
          /* a fixed, non-repeating sideways offset for the narrow lane */
          sway: Math.sin(i * 2.399) * 0.5 + Math.sin(i * 1.117) * 0.4,
          x: 0, y: 0, w: 0, h: 0, d: 0
        });
      }
    }
  ...
  function measure() {
    ...
    items.forEach(function (it) {
      it.label = (compact && it.f.short) ? it.f.short : it.f.said;
      it.textW = ctx.measureText(it.label).width;
    });
  }

  /* ── the ring ────────────────────────────────────────────────────────── */
  function place() {
    if (small) placeLane(); else placeRing();
  }

  /* Wide: one ring around the card. */
  function placeRing() {
    var base = 13;
    for (var i = 0; i < items.length; i++) {
      var it = items[i];
      var a = angle + it.phase;
      ...
      if (over > 0) {
        /* Each pill keeps its own clearance, scaled by the height it was dealt.
           A shared minimum would collapse every pushed pill onto one line; this
           way they clear the card and each other. */
        var need = card.h / 2 + it.h / 2 + 12 + Math.abs(it.lift) * H * 0.40;
  ...
  /* Narrow: a lane above the card. Each fragment drifts down it and is
     absorbed — it fades out against the card's top edge, and the card answers
     for whichever one is closest to being taken in. Same claim as the ring,
     in the one dimension a phone actually has. */
  function placeLane() {
    var top = 16, foot = card.y - card.h / 2 - 6;
    var span = Math.max(foot - top, 60);
    var best = -1, bestT = -1;

    for (var i = 0; i < items.length; i++) {
      var it = items[i];
      var t = flow + i / items.length;
      t -= Math.floor(t);                                // 0 at the top, 1 absorbed
      var hot = (i === active());

      var sc = hot ? 1.02 : 0.88 + t * 0.12;
      it.d  = t;
      it.sc = sc;
      it.h  = 24 * sc;
      it.w  = 34 * sc + it.textW * sc;
      ...
      it.y = top + t * span;
      it.x = clamp(card.x + it.sway * W * 0.16, it.w / 2 + 8, W - it.w / 2 - 8);

      /* arrives from nothing, and is gone by the time it reaches the card */
      var fade = Math.min(t / 0.12, 1) * Math.min((1 - t) / 0.16, 1);
      it.a = hot ? Math.max(fade, 0.75) : fade * 0.92;

      if (t > bestT && t < 0.9) { bestT = t; best = i; }
    }
    auto = best;
  }
  ...
  function drawPill(it, hot) {
    ...
    ctx.fillText(it.label, x + (11 + 3.4 + 8) * it.sc, it.y + 0.5);
    ctx.globalAlpha = 1;
  }
  ...
  function draw() {
    ...
    if (small) {
      /* everything is drawn before the card, so a fragment disappears into it */
      order.forEach(function (it) { if (it.i !== a) drawPill(it, false); });
      if (a >= 0 && items[a]) { drawThread(items[a], true); drawPill(items[a], true); }
      drawCard();
      return;
    }
  ...
  function frame(now) {
    ...
    if (!reduced) {
      /* One rule covers both compositions: the motion is always easing back to
         its ambient rate, so a flick decays like a flywheel, and holding a
         fragment slows everything down while you read it. The ring turns by
         it; the lane drifts by it. */
      if (!dragging) {
        var held = pinned >= 0 || hover >= 0;
        var target = held ? AMBIENT * 0.08 : AMBIENT;
        spin += (target - spin) * Math.min(1, dt * EASE_BACK);
      }
      if (small) flow += spin * dt * 0.62;
      else angle += spin * dt;
    }
  ...
  layout();
  if (reduced) { angle = 0.42; flow = 0.34; draw(); }
  start();
